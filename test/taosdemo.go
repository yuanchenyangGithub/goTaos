...
func init() {
	flag.StringVar(&configPara.hostName, "h", "izwz9cdth", "The host to connect to TDengine server.")
	...
	flag.StringVar(&configPara.dbName, "d", "devDB", "Destination database.")
	...
	configPara.days = 30
	configPara.supTblName = "t_battery_history_data"
...
func selectTest(dbName string, tbPrefix string, supTblName string) {
	...
	limit := 3
	offset := 0
	sqlStr := "select ts,soc from " + dbName + "." + supTblName + " limit " + strconv.Itoa(limit) + " offset " + strconv.Itoa(offset)
	...
	for rows.Next() {
		...
		err := rows.Scan(&ts, &soc)
		if err != nil {
			checkErr(err, "rows scan fail")
		}

		fmt.Printf("ts:%s\t soc:%d\n", ts, soc)
	}
